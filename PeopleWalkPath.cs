...
{
    private EnumMove lastMoveType = EnumMove.Walk;

    public enum EnumMove { Walk, Run };
    public enum EnumDir { Forward, Backward, HugLeft, HugRight, WeaveLeft, WeaveRight };

    [Tooltip("Type of movement / Тип движения")] [SerializeField] private EnumMove _moveType;
    ...
    [Tooltip("Speed of move / Скорость движения")] [SerializeField] private float moveSpeed = 1;
    ...
    {
        if(lastMoveType != _moveType)
        {
            lastMoveType = _moveType;

            if(lastMoveType == EnumMove.Run)
            {
                moveSpeed = 3;
    ...
    public override void SpawnOnePeople(int w, bool forward, float mSpeed)
    {
    ...
    public override void SpawnPeople()
    {
        ...
        for (int wayIndex = 0; wayIndex < numberOfWays; wayIndex++)
        {
            ...
            for (int peopleIndex = 0; peopleIndex < peopleCount; peopleIndex++)
            {
                ...
                movePath.MyStart(wayIndex, GetRoutePoint((peopleIndex + 1) * segmentLen + randomShift, wayIndex, pCount, forward, loopPath), animName, loopPath, forward, moveSpeed);
